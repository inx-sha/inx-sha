## Inshaf Ahamed
**Computer Engineering Student @ PAF-IAST, Pakistan**

🔬 Machine Learning Intern — SPCAI Robotics Lab (on-device voice assistant, TinyML)  
🛠️ Focus: FPGA/RTL Design · Embedded Systems · Embedded ML  
📌 Tools: Verilog · Vivado · ESP32 · Arduino · MATLAB  

### Projects
- [SAP-1 Extended CPU](https://github.com/inx-sha/sap1-extended-verilog) — 8-bit CPU in Verilog with ADD/SUB/MUL/DIV
- [EmotionSense](https://github.com/inx-sha/EmotionSense) — Multimodal emotion recognition (text + speech)
